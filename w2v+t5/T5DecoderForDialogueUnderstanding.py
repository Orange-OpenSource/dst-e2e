...
class T5DecoderModelForDialogueUnderstanding(nn.Module):
    ...
    def _from_pretrained(self, source, config, model, save_path):
        ...
        if is_sb:
            ...
            self.model = AutoModelForSeq2SeqLM.from_pretrained(source, cache_dir=save_path)
    ...
    def decode(
        ...
        output = self.model.decoder(input_ids=memory, encoder_hidden_states=enc_states, output_attentions=True)

        return self.model.lm_head(output.last_hidden_state), output.cross_attentions[-1]